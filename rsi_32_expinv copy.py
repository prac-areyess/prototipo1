...
class RSIRATAutomation32:
    ...
    def get_expedientes_grouped_by_dependencia(self):
        ...
        Retorna:
            ...
            - Orden de las dependencias: la que aparece primero es la que inicia
        
        Ejemplo:
            {
                "23": [fila_0, fila_1],  ← Inicia con 23 (aparece primero)
                "21": [fila_2, fila_3]
            }
        """
        try:
            ...
            for idx, fila in expedientes.iterrows():
                ...
                # Registrar el orden (solo la primera vez que aparece)
                if dep_key not in grupos:
                    orden_dependencias.append(dep_key)
                    grupos[dep_key] = []
                
                # Agregar la fila al grupo
    ...
    def process_dependencia_batch(self, dependencia, expedientes_grupo, is_first=False, is_last=False):
        ...
        try:
            ...
            for idx, (row_idx, fila) in enumerate(expedientes_grupo, 1):
                ...
                if idx == 1:
                    ...
                    # SIGUIENTES expedientes: Cambio de Expediente (REUSA FLUJO)
                    logger.info("→ Expediente siguiente en mismo lote: Usando Cambio de Expediente...")
                    resultado = self.click_cambio_expediente()
                    if resultado:
                        resultado = self.enter_expediente_field()
                        if resultado:
        ...
        while time.time() - tiempo_inicio < timeout_segundos:
            ...
            try:
                ...
                # Intentar encontrar por titulo exacto (con tolerancia a espacios)
                try:
                    cobranza_control = app_window.child_window(title="Cobranza Coactiva")
                    if cobranza_control.exists(timeout=1):
                        logger.info("Elemento encontrado por titulo exacto, haciendo clic...")
                ...
                # Si no encontrado, buscar en descendientes con tolerancia a espacios
                if not encontrado:
                    try:
                        descendants = app_window.descendants()
                        
                        # DEBUG: En el primer intento, listar todos los elementos
                        if intento == 1:
                            logger.info("=" * 70)
                            logger.info("DEBUG - LISTA DE ELEMENTOS (window_text y Name):")
                            logger.info("=" * 70)
                            for idx, desc in enumerate(descendants):
                                try:
                                    window_txt = desc.window_text().strip()
                                    # Intentar obtener el atributo Name
                                    name_attr = ""
                                    try:
                                        if hasattr(desc.element_info, 'name'):
                                            name_attr = desc.element_info.name
                                    except:
                                        pass
                                    
                                    if window_txt or name_attr:
                                        logger.info(f"[{idx}] window_text='{window_txt}' | Name='{name_attr}'")
                                except:
                                    pass
                            logger.info("=" * 70)
                        ...
                        for descendant in descendants:
                            try:
                                window_text = descendant.window_text().strip()  # Usar strip() para eliminar espacios
                                
                                # Intentar obtener el atributo Name también
                                name_attr = ""
                                try:
                                    if hasattr(descendant.element_info, 'name'):
                                        name_attr = descendant.element_info.name
                                except:
                                    pass
                                
                                # Buscar en window_text O en Name
                                if ("Cobranza Coactiva" in window_text or window_text.startswith("Cobranza Coactiva") or
                                    "Cobranza Coactiva" in name_attr or name_attr.startswith("Cobranza Coactiva")):
                                    logger.info(f" Elemento encontrado: window_text='{window_text}' | Name='{name_attr}'")
                                    rect = descendant.rectangle()
                                    if rect.left > 0 or rect.top > 0:
                                        center_x = (rect.left + rect.right) // 2
                                        center_y = (rect.top + rect.bottom) // 2
        ...
        logger.warning("No se pudo encontrar 'Cobranza Coactiva'")
        return False
    ...
    def run(self):
        ...
        Flujo:
        1. Agrupa expedientes por dependencia (21 y 23)
        2. Abre la aplicación UNA SOLA VEZ
        3. Para cada dependencia en orden:
